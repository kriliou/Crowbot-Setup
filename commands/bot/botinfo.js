...
module.exports = {
	...
	run: async (client, message, args, prefix, color) => {
		...
		if (client.config.owner.includes(message.author.id)) {
            ...
            embed.setURL('https://discord.gg/caN6P2uYvK')
            embed.setDescription('**Une description complète et détaillée du Bot** <@1249433930067611681>')
            embed.setColor(color)
            .setThumbnail(message.author.avatarURL({ dynamic:true }));
            embed.setTimestamp()
            embed.setFooter(`${client.config.name}`)
            embed.addFields(
                { name: '👑 Owner / Developer :', value: '<@1249433930067611681>', inline: true },
                ...
                { name: '📞 Support :', value: `[Clique ICI Pour Rejoindre Le Support](https://discord.gg/caN6P2uYvK)`, inline: true },
